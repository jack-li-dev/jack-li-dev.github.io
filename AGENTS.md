...
- Native-Chinese review must also include a mandatory verbosity/information-density pass. Technical prose should say each fact once, with the shortest **natural and fluent** wording that preserves causality, scope, evidence strength, and necessary teaching context. Remove repeated premises, duplicate explanations, synonym stacking, editor narration, and obvious consequences the reader can already infer. Prefer the shorter idiomatic form when meaning and rhythm are unchanged (`常` may be better than `经常` in a compact technical sentence), but do not mechanically delete words that carry real discourse structure. A slightly longer connector such as `首先` / `但` / `所以` / `虽然…但…` is correct when it makes ordering, causality, contrast, concession, progression, or parallel structure genuinely clearer. For every sentence and paragraph, ask both questions: `can this be shorter without sounding clipped?` and `does this need one more connective or particle to read as natural Chinese?` Concision and fluency are co-equal gates; when they conflict, choose the shortest version that still reads smoothly and expresses the real logical relation. Do not chase an arbitrary word-count target or delete qualifiers/evidence merely to look concise.
...
- Reader-facing technical articles must also pass a semantic Markdown architecture gate. Blank lines only separate paragraphs; they do not by themselves create document architecture. Use Markdown structures according to information type: `##` for a major narrative phase, `###` for a stable subproblem/branch/responsibility inside that phase, tables for side-by-side environment/option comparisons, lists for peer items or checklists, blockquotes for a focused question/warning/quoted claim, and code/ASCII for executable behavior or system/resource flow. Do not leave a long article as `H2 + prose + blank line + prose` when the content contains real branches that deserve hierarchy. Conversely, do not create a heading for every paragraph or turn prose into decorative lists just to look structured. The test is whether a reader can scan the Markdown and recover the article's technical architecture before reading every sentence.
- Public article Markdown must not contain reader-facing Obsidian `[[Note]]` / `[[Note#Heading|Alias]]` residue. Knowledge Canonical Wiki links are renderer-specific source syntax; publication/export must convert them to ordinary Markdown links or remove the navigation if it has no public target. Fenced/inline code may mention Wiki syntax only when the syntax itself is the subject. The prepublish validator must fail closed on leaked Wiki links even if the manifest's `links` gate was manually marked PASS.
- Treat heading semantics as part of that architecture gate. Reader-facing H2/H3 should name an object, state, phenomenon, responsibility, question boundary, or evidence-backed conclusion; author-control labels such as `先跑一段代码`, `先修最小程序`, `下一步`, or a bare `可运行入口` do not become professional merely because they are headings. A child heading must add information beyond its parent; repeated parent/child wording should be merged, demoted, or renamed. If H3 density starts to resemble a directory tree, fold peer facts back into prose, lists, or a genuinely comparative table.
- Table choice must survive both desktop and mobile reading. Keep compact tables when readers truly need side-by-side comparison; do not use a wide table merely to create an architectural look. Navigation tables, long-link tables, or prose-heavy tables that would force horizontal scanning on a narrow viewport should become vertical lists, grouped sections, or key/value blocks. Code, ASCII, command output, and prose must remain visually distinct. Production/troubleshooting sections should retain a runbook shape such as `symptom/object -> observation -> decision branch -> action -> verification/rollback`, rather than collapsing into narrative prose.
- Reproducible debugging, benchmark, version-sensitive, or environment-sensitive articles must front-load a compact verification baseline before the first behavior explanation when OS/runtime/library versions materially affect the conclusion. Prefer a small Markdown table containing only load-bearing context such as OS/architecture, language/runtime version, framework/library version, deployment mode or resource type, and the scope each item constrains. Do not dump every transitive dependency or kernel detail into the opening; detailed manifests, checksums, and commands belong in the later reproduction section. Avoid duplicating the same environment block twice: the opening establishes the reader-facing boundary, while the later section carries exact reproduction artifacts.
- When an article uses deployment/environment examples, cover the major execution layers that materially change the answer instead of silently treating one platform as the default. Unless the topic specifically requires another order, orient readers from host-level deployment to containerization to orchestration: bare metal / VM -> Docker -> Kubernetes -> any cross-environment special branch such as application-managed files. Within each layer, mention materially different logging/resource patterns when they affect the conclusion (for example systemd/journald versus app-owned files on hosts). Keep ecosystem-prevalence claims out of the article unless independently evidenced; the ordering rule is about environment coverage and conceptual progression, not an unsupported claim that one platform is universally more common.
